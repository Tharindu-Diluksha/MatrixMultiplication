...
main(int argc, char *argv[]) {
    ...
    for (int i=200;i<=2000;i+=200){ // call through this loop for each test case
        std::cout<<i<<"\n";
...
void CreateFullMatrix(double** matrix, int n){ // Create dynamic matrix according to the n value
    for (int i=0;i<n;i++){
        matrix[i] = new double[n];
    }
}

void FillMatrix (double** matrix, int n){ // Random generated double values for matrix
    for(int i = 0; i < n; ++i){
        for(int j = 0; j < n; ++j){           
...
void FreeMatrix (double** matrix, int n){ // Delete matrix memory
    for(int i = 0; i < n; ++i){
        delete [] matrix[i];
    }
    delete [] matrix;
...
void StartTime(){ //Set the start time
    clock_gettime(CLOCK_MONOTONIC, &begin);
}

void StopTime(){ //Set the end time
    clock_gettime(CLOCK_MONOTONIC, &end);
}

double GetTime(){ // Get the time difference between start and end time
